...
if( valid_string($_REQUEST["action"]) ) {
  ...
  switch( $_REQUEST["action"] ) {
    ...
    case "insert":
      ...
      if(ereg("^.+@.+\..+$", $_POST["email"] ) ) $email = safe_data($_POST["email"]);
      else warning($lang["invalid_email"], sprintf( $lang["invalid_email_given_sprt"], $_POST["email"]) );
      ...
      $q = db_query("INSERT
                     INTO users(name, fullname, password, email, admin, deleted)
      ...
      if( $usergroup_names == "" ) $usergroup_names = $lang["not_usergroup"]."\n";
      if( $admin_rights == "t" ) $admin_state = $lang["admin_priv"]."\n";
   ...
   case "edit":
      ...
      //check email address
      if(ereg("^.+@.+\..+$", $_POST["email"] ) ) $email = safe_data($_POST["email"]);
      else warning($lang["invalid_email"], sprintf($lang["invalid_email_given_sprt"], safe_data($_POST["email"]) ) );
	...
	if( $usergroup_names == "" ) $usergroup_names = $lang["not_usergroup"]."\n";
	if( $password == "" ) $password = $lang["no_password_change"];
	if( $admin_rights == "t" ) $admin_state = $lang["admin_priv"]."\n";
